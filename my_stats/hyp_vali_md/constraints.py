...
def check_hyp_min_sample(n: int, p: int = None):
    exc = lambda st, nb: Exception(
        f"pb: {st} should be at least {LIM_MIN_SAMPLE}. Instead, found {nb}\nNote that there are several ways to bypass this"
    )
    if p == None:
        if n < LIM_MIN_SAMPLE: raise exc("n", n)
    if p != None:
        if n * p < LIM_MIN_SAMPLE: exc("n*p", n * p)
        if n * (1 - p) < LIM_MIN_SAMPLE: exc("n*(1-p)", n * (1 - p))
